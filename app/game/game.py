...
def create_action_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Стрелять в себя",
                              callback_data="shoot_self")],
        [InlineKeyboardButton(text="Стрелять во врага",
                              callback_data="shoot_enemy")],
        [InlineKeyboardButton(text="Крутить обойму", callback_data="spin")]
    ])
...
def create_main_menu_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Начать новую игру",
                              callback_data="start_new_game")]
    ])


async def start_game(users_in_game, chamber_count=7):
    game_id = f"{users_in_game[0][0]}_{users_in_game[1][0]}"
    game_states[game_id] = {
        "users": users_in_game,
        "shooter_count": {user[0]: 0 for user in users_in_game},
        # Инициализация позиции пули
        "revolver_position": random.randint(1, chamber_count),
...
@router.callback_query(F.data.in_(["shoot_self", "shoot_enemy", "spin"]))
...
async def perform_action(callback: CallbackQuery, action_type: str):
    ...
    game_id = next((gid for gid, users in game_states.items() if user_id in [
                   user[0] for user in users["users"]]), None)
    ...
    shooter, opponent = (users_in_game[0], users_in_game[1]) if user_id == users_in_game[0][0] else (
        users_in_game[1], users_in_game[0])

    # Увеличиваем счетчик спусков
    
    shooter_count = game_states[game_id]["shooter_count"][shooter[0]]
    ...
    if game_states[game_id]["shooter_count"][shooter[0]] > game_states[game_id]["max_turns"]:
        await declare_result(opponent, shooter, game_id, "max_shoot")
        return
    ...
    if action_type == "self":
        ...
        game_states[game_id]["revolver_position"] = random.randint(
            1, 6)  # Изменение позиции пули
        game_states[game_id]["max_turns"] += 5
...
async def process_self_shooting(shooter, opponent, game_id):
    if game_states[game_id]["revolver_position"] == random.randint(1, 7):  # Проверка, попал ли shooter
        await declare_result(shooter, opponent, game_id, "self_shoot")
    else:
...
async def process_enemy_shooting(shooter, opponent, game_id):
    if game_states[game_id]["revolver_position"] == random.randint(1, 7):  # Проверка, попал ли opponent
        await declare_result(opponent, shooter, game_id, "enemy_shoot")
    else:
...
async def declare_result(loser, winner, game_id, result_type):
    ...
    elif result_type == "enemy_shoot":
        await bot.send_message(winner[0], "🏆 Вы победили! Игра окончена.")
        await bot.send_message(loser[0], "💀 Противник навёл ствол револьвера на вашу голову и спустил курок, всë резко потемнело. Вы мертвы.")
